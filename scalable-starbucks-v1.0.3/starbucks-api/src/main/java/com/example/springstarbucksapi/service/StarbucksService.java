...
public class StarbucksService {
    ...
    public StarbucksOrder getActiveOrder(String regid) {
        ...
        if (!orders.isEmpty()) {
            return orders.get(0); // return the first item in the list
        }
        return null;
    ...
    public void clearActiveOrder(String regid) {
        ...
        // For each order, set the order of each associated drink to null
        for (StarbucksOrder order : orders) {
            for (StarbucksDrink drink : order.getDrinks()) {
                drink.setOrder(null);
                // Save the drink
    ...
    public StarbucksCard processOrder(String regid, String cardnum) throws ResponseStatusException {
        ...
        rabbit.convertAndSend(starbucksQueue.getName(), drink.getDrinkID().toString());
        return card;
    }
